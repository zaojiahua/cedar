<template>
    <div>
        <Input class="search-x" search enter-button="Search" clearable v-model="keyword" @on-change="onChange" :placeholder="$t('rdsTboardDeviceStatistic.tips_3')" style="margin-bottom: 16px" />
        ...
        <!-- RDS  数据 部分 -->
        <Divider orientation="left" style="margin-bottom: 16px" v-show="showRdsCard">{{$t('rdsDetail.result')}}</Divider>
        <Card :bordered="false" :dis-hover="true" style="overflow:hidden;" v-if="showRdsCard">
            <p style="font-size: 12px"><span>{{$t('rdsDeviceStatistic.dev')}}：【{{ device.name }}】</span><span v-show="job.name" style="margin-left: 20px;">{{$t('rdsDeviceStatistic.job')}}：【{{ job.name }}】</span></p>
